...
export class AuthService {
  ...
  registerUser(formData: Register): Observable<{ user: any }> {
    return this.http.post<{ user: any }>('https://pizzeria-auth.onrender.com/user/register', formData);
  }

  loginUser(email: string, password: string): Observable<TokenResponse> {
    return this.http.put<TokenResponse>('https://pizzeria-auth.onrender.com/user/login', { email, password }).pipe(
      tap({
        next: (resData) => {
  ...
  refreshToken(refreshToken: string) {
    return this.http.post<TokenResponse>('https://pizzeria-auth.onrender.com/user/refresh-token', { refreshToken }).pipe(
      tap((resData) => {
        this.handleAuthentication(resData);
      })
    ).subscribe({
